...
class VllmModelIndicator(ChatModelIndicator):
    def __init__(self, 
                 url: str, 
                 model: str, 
                 api_key: str, 
    ...
    def create_model(self, logger: ChatLogger, max_tokens: int | None = None) -> VllmModel:
        if max_tokens is None or max_tokens > self.max_tokens():
            max_tokens = self.max_tokens()
        return VllmModel(self.url(), self.model(), self.api_key(), 
