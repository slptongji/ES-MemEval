...
from lib.sum.sum_experiment import SumExperiment
...
def memory(parameters: SumExperimentParameters.MemoryStrategyParameters):
    embedding: HuggingFaceEmbeddings = EmbeddingProvider.bge_m3(parameters.prefer_cuda)
    store: DocumentStore = VectorDocumentStore(4, "FAISS", embedding)
    strategy: PromptStrategy = SessionWiseMemoryInplaceStrategy(store, True)
    return strategy
...
def main():
    ...
    experiment: SumExperiment = SumExperiment(SumExperimentParameters(
        output_directory, Config.model, 
        16000, Config.judge_model,
...
if __name__ == "__main__":
    class Config:
        ...
        model: ChatModelIndicator = common_configurations.gpt4o
